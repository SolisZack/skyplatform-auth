...
func (userService *UserService) GetUserInfoList() (error, []model.SysUser, int64) {
	var users []model.SysUser
	result := global.DB.Find(&users)
	return result.Error, users, result.RowsAffected
	//limit := info.PageSize
...
func (userService *UserService) SetUserAuthority(uuid uuid.UUID, authorityId string) (err error) {
	assignErr := global.DB.Where("uuid = ?", uuid).First(&model.SysUser{}).Error
...
func (userService *UserService) SetUserInfo(reqUser model.SysUser) (err error, user model.SysUser) {
	err = global.DB.Where("username = ?", reqUser.Username).Updates(&reqUser).Error
	return err, reqUser
}
...
func (userService *UserService) GetUserInfo(uuid uuid.UUID) (err error, user model.SysUser) {
	...
	err = global.DB.First(&reqUser, "uuid = ?", uuid).Error
	return err, reqUser
}
